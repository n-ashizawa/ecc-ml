...
class BertModel(BertPreTrainedModel):
    ...
    def get_prune_layers_and_output(
        ...
        if not return_dict:
            return (steps, (sequence_output, pooled_output) + encoder_outputs[1:])
...
class BertEmbeddings(nn.Module):
    ...
    def get_prune_layers_and_output(self,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.word_embeddings(input_ids)
            steps.append((self.word_embeddings, inputs_embeds))
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        steps.append((self.token_type_embeddings, token_type_embeddings))
...
class BertEncoder(nn.Module):
    ...
    def get_prune_layers_and_output(
        ...
        final_output = BaseModelOutputWithPastAndCrossAttentions(
            last_hidden_state=hidden_states,
            past_key_values=next_decoder_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attentions,
...
class BertLayer(nn.Module):
    ...
    def get_prune_layers_and_output(
        ...
        step, layer_output = apply_chunking_to_forward(
            self.exfeed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
        steps.extend(step)
        outputs = (layer_output,) + outputs
...
class BertSelfAttention(nn.Module):
    ...
    def get_prune_layers_and_output(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            positional_embedding = self.distance_embedding(distance + self.max_position_embeddings - 1)
            steps.append((self.distance_embedding, positional_embedding))
            positional_embedding = positional_embedding.to(dtype=query_layer.dtype)  # fp16 compatibility
