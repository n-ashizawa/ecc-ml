...
from numpy import array, zeros, exp, log, empty, concatenate, split

from .conv_code import conv_encode
...
def turbo_encode(msg_bits, trellis1, trellis2, interleaver):
    ...
    Encode Bits using a parallel concatenated rate-1/3(or 1/5)
    turbo code consisting of two rate-1/2(or 1/3) systematic
    ...
    """
    rate = float(trellis1.k) / trellis1.n
    # ビットストリームに対し、RSC符号により、符号化されたビットストリームm+e1...ekを得る
    stream = conv_encode(msg_bits, trellis1, 'rsc', puncture_matrix=None)
    if rate == 1/3:
        sys_stream = stream[::3]
        e1 = stream[1::3]
        e2 = stream[2::3]
    ...
    # インターリーバにより、ビットストリームの並べ替えを行う
    interlv_msg_bits = interleaver.interlv(sys_stream)

    # 並べ替えられたビットストリームに対し、RSC符号により、m_bar+e_bar1...e_barkを得る
    stream_2 = conv_encode(interlv_msg_bits, trellis2, 'rsc', puncture_matrix=None)
    if rate == 1/3:
        e_bar1 = stream_2[1::3]
        e_bar2 = stream_2[2::3]
    else:
    ...
    if rate == 1/3:
        non_sys_stream_1 = concatenate(
            [e1[0:-trellis1.total_memory], e2[0:-trellis1.total_memory]]
            )
        non_sys_stream_2 = concatenate(
            [e_bar1[0:-trellis2.total_memory], e_bar2[0:-trellis2.total_memory]]
            )
    else:
        non_sys_stream_1 = e1[0:-trellis1.total_memory]
        non_sys_stream_2 = e_bar1[0:-trellis2.total_memory]

    return [sys_stream, non_sys_stream_1, non_sys_stream_2]


def _compute_branch_prob(code_bit_0, code_bit_1, code_bit_2, rx_symbol_0, rx_symbol_1, rx_symbol_2,
                         noise_variance):
    ...
    if code_bit_2 is not None and rx_symbol_2 is not None:
        code_symbol_2 = 2*code_bit_2 - 1
        z = rx_symbol_2 - code_symbol_2
    else:
        z = 0.0

    # Normalized branch transition probability
    branch_prob = exp(-(x*x + y*y + z*z)/(2*noise_variance))
    ...
    for reverse_time_index in reversed(range(1, msg_length+1)):
        ...
        for current_state in range(number_states):
            for current_input in range(number_inputs):
                ...
                if rate == 1/3:
                    parity_bit_2 = codeword_array[2]
                    e1, e2 = split(non_sys_symbols, 2)
                    rx_symbol_1 = e1[reverse_time_index-1]
                    rx_symbol_2 = e2[reverse_time_index-1]
                else:
                    parity_bit_2 = None
                    rx_symbol_1 = non_sys_symbols[reverse_time_index-1]
                    rx_symbol_2 = None
                branch_prob = _compute_branch_prob(msg_bit, parity_bit, parity_bit_2,
                                                   rx_symbol_0, rx_symbol_1, rx_symbol_2,
                                                   noise_variance)
...
def map_decode(sys_symbols, non_sys_symbols, trellis, noise_variance, L_int, mode='decode'):
    ...
    (rate 1/2(or 1/3)) bits using the MAP algorithm.
...
def turbo_decode(sys_symbols, non_sys_symbols_1, non_sys_symbols_2, trellis,
    ...
    Decodes a stream of convolutionally encoded
    (rate 1/3(or 1/5)) bits using the BCJR algorithm.
    ...
    # 2つのMAP Decoderを繰り返し利用する
    for iteration_count in range(number_iterations):

        # MAP Decoder - 1
        # この内部でBCJRアルゴリズムが使われている
        # BCJRアルゴリズムとは、誤り検出訂正（主に畳み込み符号）の事後確率最大化復号のアルゴリズム
